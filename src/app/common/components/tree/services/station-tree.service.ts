...
export class StationTreeService {
  ...
  constructor(private _divisionTreeService: DivisionTreeService, private _stationRequest: GarbageStationRequestService, private _converter: TreeConverter) { }
  getName() {
    ...
    if (condition != '') {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (station.DivisionId) {
          ...
          let division = await this._divisionTreeService.getDivision(station.DivisionId)
          allDivisions.push(division)
          let ancestors = await this._divisionTreeService.getAncestorDivision(division)
