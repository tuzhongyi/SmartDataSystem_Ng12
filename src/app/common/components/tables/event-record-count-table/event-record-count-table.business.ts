...
import { GetGarbageStationsParams, GetGarbageStationStatisticNumbersParamsV2 } from "src/app/network/request/garbage-station/garbage-station-request.params";
import { GarbageStationRequestService } from "src/app/network/request/garbage-station/garbage-station-request.service";
import { IntervalParams } from "src/app/network/request/IParams.interface";
import { EventRecordCountTableConverter } from "./event-record-count-table.converter";
import { EventRecordCountTableModel, EventRecordCountTableOptions, StatisticType } from "./event-record-count-table.model";

@Injectable()
export class EventRecordCountTableBusiness implements IBusiness<StatisticType[], EventRecordCountTableModel[]>
{
    constructor(
    ...
    async load(opts: EventRecordCountTableOptions): Promise<EventRecordCountTableModel[]> {
        let id = opts.id
        if(!id)
        {
    ...
    async getData(id:string, type:UserResourceType, opts: EventRecordCountTableOptions): Promise<StatisticType[]> {
        ...
        if (opts.type === UserResourceType.Station) {
            ...
            return this.getDivisionData(id, ids, interval, opts.unit)
        }
    }
    ...
    getGarbageStationData(stationIds: string[], interval: IntervalParams, unit: TimeUnit) {
    ...
    getDivisionData(divisionId: string, divisionIds: string[], interval: IntervalParams, unit: TimeUnit) {
