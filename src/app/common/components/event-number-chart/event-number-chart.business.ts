...
export class EventNumberChartBusiness {
  ...
  private async _getData() {
    let beginTime = TimeService.beginTime(new Date());
    let endTime = TimeService.endTime(new Date());
    ...
    let res = await this._divisionRequest.eventNumber.history.list(
      '310109000000',
      params
    );
    console.log(res);
  ...
  async getByStation() {
    ...
    let res = await this._garbageStationRequest.eventNumber.history.list(
      '310109009002001000',
      params
    );
    console.log(res);
