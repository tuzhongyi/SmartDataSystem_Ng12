...
@Injectable()
export class AuditStatisticDataImageWindow extends WindowViewModel {
  constructor(
    private service: AuditStatisticDataService,
    private video: AuditStatisticDataVideoWindow
  ) {
    super();
  }
  ...
  private async manualCapture(
    ...
    return timer(1000)
      ...
      .then((x) => {
        if (stationId) {
          return this.service.station.manualCapture(stationId).then((urls) => {
            try {
              if (models) {
