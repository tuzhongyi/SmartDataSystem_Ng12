...
@Injectable()
export class AMapPointStationViewBusiness implements IAMapPointViewBusiness {
  ...
  async normal(show: boolean) {
    ...
    let dropIds = this.amap.source.drop.garbage.map((x) => x.Id);
    let stations = this.amap.source.garbage.filter(
  ...
  full(show: boolean) {
    ...
    let dropIds = this.amap.source.drop.garbage.map((x) => x.Id);
    let stations = this.amap.source.garbage.filter((x) => {
  ...
  async drop(show: boolean) {
    ...
    let stations = this.amap.source.drop.garbage.map((x) => x.GarbageStation);
  ...
  private async plugarc(show: boolean, query: (time?: number) => boolean) {
    let models = this.amap.source.drop.garbage;

    if (show) {
      this.drop(show);
      let drops = models.filter((x) => query(x.CurrentGarbageTime));
      let surplus = this.amap.source.plug.garbage.filter((x) => {
        return (
