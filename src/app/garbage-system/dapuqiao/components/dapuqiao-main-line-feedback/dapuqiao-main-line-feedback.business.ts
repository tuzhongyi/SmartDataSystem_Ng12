...
import { DaPuQiaoMainLineFeedbackModel } from './dapuqiao-main-line-feedback.model';

@Injectable()
export class DaPuQiaoMainLineFeedbackBusiness
  implements
    IBusiness<DivisionNumberStatisticV2[], DaPuQiaoMainLineFeedbackModel>
{
  constructor(
    ...
    for (let i = 0; i < datas.length; i++) {
      ...
      if (
        ...
        if (data.Level3Statistic) {
          one.value.push(data.Level3Statistic.Level1FeedbackNumber ?? 0);
          two.value.push(data.Level3Statistic.Level2FeedbackNumber ?? 0);
          three.value.push(data.Level3Statistic.Level3FeedbackNumber ?? 0);
        }
      }
    }

    let array: DaPuQiaoMainLineFeedbackModel = {
      x: x,
      datas: [one, two, three],
    };
    return array;
