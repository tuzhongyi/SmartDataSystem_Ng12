...
export class StatisticToTimeDataConverter implements IConverter<EventNumberStatistic[], ITimeData<number>[]>{
    Convert(source: EventNumberStatistic[], eventType:EventType): ITimeData<number>[] {
      return source.map(x=>{
        let count = 0;
