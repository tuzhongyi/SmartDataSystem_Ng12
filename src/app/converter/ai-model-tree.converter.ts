...
import { CameraAIModelDTOLabel, EnumValue } from "src/app/network/model/camera-ai.model";
...
export class AIModelTreeConverter {
  ...
  recurseToNestTreeNode<T extends AIModelTreeModel>(data: T[], parentId: string | null = null) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (item instanceof CameraAIModelDTOLabel) {
        ...
        if (!item.IsLeaf && item.Labels) {
          ...
          let children = this.recurseToNestTreeNode(item.EnumValues, item.LabelId);
          node.childrenChange.next(children);
        }
  ...
  private _fromCameraAIModelDTOLabel(item: CameraAIModelDTOLabel) {
    ...
    node.hasChild = !item.IsLeaf || !!(item.IsLeaf && item.EnumValues && item.EnumValues.length > 0);
