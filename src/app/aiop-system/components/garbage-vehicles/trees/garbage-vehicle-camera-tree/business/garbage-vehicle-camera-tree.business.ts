...
import { GarbageVehicleCameraTreeDivisionBusiness } from './garbage-vehicle-camera-tree-division.business';
import { GarbageVehicleCameraTreeVehicleBusiness } from './garbage-vehicle-camera-tree-vehicle.business';
...
export class GarbageVehicleCameraTreeBusiness implements IDivisionTreeBusiness {
  ...
  public display = [Division, GarbageVehicle, VehicleCamera];

  public nestedNodeMap = new Map<string, CommonNestNode<Division>>();

  constructor(
    private _division: GarbageVehicleCameraTreeDivisionBusiness,
    private _vehicle: GarbageVehicleCameraTreeVehicleBusiness,
    private _camera: GarbageVehicleCameraTreeCameraBusiness,
    private _converter: DivisionTreeConverter
  ) {}
  async getData(): Promise<DivisionTreeSource[]> {
    let vehicles = await this._vehicle.all();
    let divisions = await this._division.all();
    let cameras = await this._camera.all();
    return [...divisions, ...vehicles, ...cameras];
  }
