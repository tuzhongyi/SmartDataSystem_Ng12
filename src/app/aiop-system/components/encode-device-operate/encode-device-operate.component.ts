...
export class EncodeDeviceOperateComponent implements OnInit {
  ...
  myForm = this.fb.group({
    ...
    Port: ['', [Validators.required, Validators.pattern(/^\d*$/), Validators.max(65535), Validators.min(0)]],
    Username: [''],
    Password: [''],
  ...
  constructor(private fb: FormBuilder, private _business: EncodeDeviceOperateBusiness, private _toastrService: ToastrService) { }
    ...
    if (this._checkForm()) {
      if (this.state == FormState.add) {
        ...
        if (this._encodeDevice) {
          ...
          this._encodeDevice.Url = "http://" + this.myForm.value.Hostname + ":" + this.myForm.value.Port;
